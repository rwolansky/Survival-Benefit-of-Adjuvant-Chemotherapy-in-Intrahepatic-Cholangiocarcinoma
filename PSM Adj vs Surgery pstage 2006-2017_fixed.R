...
# Load data
my_data <- read.csv("Y:/Rachel Wolansky/Intrahepatic Cholangio/Data Out/Step 4 Adj vs Surgery pstage 2006-2017.csv")

# Define covariates
all_covariates <- c("AGE", "RACE", "SEX", "INSURANCE_STATUS", "CDS", "margins", "pTStage", "xrt", "LAD")
...
print("\n=== PRE-MATCHING SURVIVAL ANALYSIS ===")
print("Prognostic factor analysis in the entire unmatched cohort")
...
cox_model_prematch <- coxph(
  Surv(time = lastcontact_5yr, event = mortality_5yr) ~ 
    ...
    factor(pTStage) + factor(xrt) + factor(LAD),
  data = my_data_5yr
)
...
psmatch <- matchit(formula, data=my_data, method="nearest", estimand="ATT", ratio=1, caliper=0.5)
...
if(length(imbalanced_vars) > 0) {
  ...
  if(length(exact_candidates) > 0) {
    ...
    psmatch_exact <- matchit(formula, data=my_data, method="nearest", estimand="ATT", 
                             ratio=1, caliper=0.5, exact=exact_candidates)
...
if(length(imbalanced_vars) > 0) {
  ...
  if(length(exact_candidates_2) > 0) {
    ...
    psmatch_exact2 <- matchit(formula, data=my_data, method="nearest", estimand="ATT", 
                              ratio=1, caliper=0.5, exact=all_exact_vars)
...
if(length(imbalanced_vars) > 0) {
  ...
  if(length(exact_candidates_3) > 0) {
    ...
    psmatch_exact3 <- matchit(formula, data=my_data, method="nearest", estimand="ATT", 
                              ratio=1, caliper=0.5, exact=all_exact_vars_final)
...
cox_model_mv_stratified <- coxph(
  Surv(time = lastcontact_5yr, event = mortality_5yr) ~ 
    adjchemo + AGE + factor(RACE) + factor(SEX) + 
    factor(INSURANCE_STATUS) + CDS + factor(margins) + 
    factor(pTStage) + factor(xrt) + factor(LAD) + strata(pair_id),
  data = matched_data_5yr
)
