...
# Load data
my_data <- read.csv("Y:/Rachel Wolansky/Intrahepatic Cholangio/Data Out/Step 4 Adj vs Surgery pstage 3 and 4.csv")

# Define covariates
all_covariates <- c("AGE", "RACE", "SEX", "INSURANCE_STATUS", "CDS", "margins", "xrt")
...
if(length(imbalanced_vars) > 0) {
  ...
  if(length(exact_candidates_2) > 0) {
    ...
    psmatch_exact2 <- matchit(formula, data=my_data, method="nearest", estimand="ATT", 
                              ratio=1, caliper=0.35, exact=all_exact_vars)
...
if(length(imbalanced_vars) > 0) {
  ...
  if(length(exact_candidates_3) > 0) {
    ...
    psmatch_exact3 <- matchit(formula, data=my_data, method="nearest", estimand="ATT", 
                              ratio=1, caliper=0.4, exact=all_exact_vars_final)
...
cox_model_mv_stratified <- coxph(
  Surv(time = lastcontact_5yr, event = mortality_5yr) ~ 
    adjchemo + AGE + factor(RACE) + 
    factor(INSURANCE_STATUS) + CDS +
