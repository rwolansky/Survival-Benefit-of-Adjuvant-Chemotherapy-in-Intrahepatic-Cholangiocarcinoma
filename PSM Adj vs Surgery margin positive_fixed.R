...
print("\n=== PRE-MATCHING SURVIVAL ANALYSIS ===")
print("Prognostic factor analysis in the entire unmatched margin-positive cohort")
...
cox_model_prematch <- coxph(
  Surv(time = lastcontact_5yr, event = mortality_5yr) ~ 
    ...
    factor(INSURANCE_STATUS) + CDS + factor(pStage) + 
    factor(xrt),
  data = my_data_5yr
)
...
# Step 1: Initial matching with caliper
print("\n=== STEP 1: INITIAL MATCHING WITH CALIPER ===")
formula <- as.formula(paste("adjchemo ~", paste(all_covariates, collapse = " + ")))
psmatch <- matchit(formula, data=my_data, method="nearest", estimand="ATT", caliper = 0.21, ratio=1)
...
# Step 2: Exact matching on imbalanced categorical variables if needed
if(length(imbalanced_vars) > 0) {
  ...
  if(length(exact_candidates) > 0) {
    ...
    psmatch_exact <- matchit(formula, data=my_data, method="nearest", estimand="ATT", 
                             ratio=1, caliper=0.25, exact=exact_candidates)
...
km_plot <- ggsurvplot(
  ...
  palette = c("blue", "red"),
  title = "Overall Survival by Adjuvant Chemotherapy Status (Margin-Positive)",
  xlim = c(0, 70),
